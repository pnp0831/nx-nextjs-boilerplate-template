...
jest.mock('@esp/constants/config', () => ({
  common: { useMockData: false, usePermission: true },
  server: {
    nextAuthSecret: 'toRzXSUUBfVKp_hmDDhEpw-ujnkGepIpDkQplaTy',
    nextAuthUrl: 'http://localhost:4242',
    nextAuthName: 'next-auth.session-token',
    oktaClientId: '0oa9v55cuuYNZLItL5d7',
    oktaClientSecret: 'toRzXSUUBfVKp_hmDDhEpw-ujnkGepIpDkQplaTy',
    oktaIssuer: 'https://dev-82915868.okta.com/oauth2/default',
    apiUrl: 'http://esp-backend-dev.southeastasia.cloudapp.azure.com',
    needToUseContentSecurity: false,
  },
  client: {
    gtmId: 'GTM-TZ25FNL',
    measurementId: 'GTM-TZ25FNL',
    apiUrl: 'http://esp-backend-dev.southeastasia.cloudapp.azure.com',
...
describe('withProxyApi', () => {
  ...
  test('should rewrite the request and attach authorization header if it match with "/api/:path*" and expect "/api/auth"', async () => {
    ...
    expect(result).toBeInstanceOf(NextResponse);
    expect(result?.headers.get('x-middleware-rewrite')).toBeDefined();
  });
...
describe('withAuthorization', () => {
  ...
  test('continues to the next middleware and includes token renewal when token refresh invalid', async () => {
    const request = new NextRequest('http://localhost:4242/api/time-management');
    const renewedToken = await encode({
      secret: 'toRzXSUUBfVKp_hmDDhEpw-ujnkGepIpDkQplaTy',
      token: {
        accessToken:
    ...
    const renewedTokenAfterRefresh = {
    ...
    (refreshToken as jest.Mock).mockResolvedValue({ data: renewedTokenAfterRefresh });
  ...
  test('continues to the next middleware and includes token renewal when token refresh invalid and redirect to home', async () => {
    const request = new NextRequest('http://localhost:4242/login');
    const renewedToken = await encode({
      secret: 'toRzXSUUBfVKp_hmDDhEpw-ujnkGepIpDkQplaTy',
      token: {
